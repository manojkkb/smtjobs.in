...
class EducationLevelController extends MasterResourceController
{
    protected string $modelClass = EducationLevel::class;
    protected string $routeName = 'admin.education-levels';
    protected string $resourceLabel = 'Education Level';
    protected string $resourcePlural = 'Education Levels';
    protected string $indexSubtitle = 'Manage the education levels that applicants can select.';
    protected string $formSubtitle = 'Ensure each level has a slug, label, and sort order before toggling availability.';
    ...
    {
        $slugRule = Rule::unique('education_levels', 'slug');
    ...
    public function import(Request $request)
    {
        ...
        foreach ($rows as $row) {
            ...
            EducationLevel::updateOrCreate(['slug' => $slug], $payload);
            $imported++;
        }

        return back()->with('status', "Imported {$imported} education levels.");
    }

    public function export(Request $request)
    {
        $columns = ['slug', 'label', 'sort_order', 'is_active'];
        $filename = 'education-levels-' . Carbon::now()->format('Y-m-d') . '.csv';
    ...
    {
        $columns = ['slug', 'label', 'sort_order', 'is_active'];
        $filename = 'education-levels-sample.csv';

        $callback = function () use ($columns) {
            $handle = fopen('php://output', 'w');
            fputcsv($handle, $columns);
            fputcsv($handle, ['high-school', 'High School', 1, 1]);
            fputcsv($handle, ['bachelors', 'Bachelors', 2, 1]);
            fclose($handle);
        };
