...
class ShiftTypeController extends MasterResourceController
{
    protected string $modelClass = ShiftType::class;
    protected string $routeName = 'admin.shift-types';
    protected string $resourceLabel = 'Shift Type';
    protected string $resourcePlural = 'Shift Types';
    protected string $indexSubtitle = 'Define the shift windows available for scheduling.';
    protected string $formSubtitle = 'Specify start/end times and priority before toggling availability.';
    ...
    protected array $sortable = ['id', 'label', 'start_time', 'end_time', 'slug', 'sort_order', 'is_active'];
    protected array $bulkActions = [
        'sample' => [
            'label' => 'Download sample CSV',
            'route' => 'admin.shift-types.sample',
            'description' => 'Use the sample to match the column order.',
        ],
        'export' => [
            'label' => 'Export filtered list',
            'route' => 'admin.shift-types.export',
            'description' => 'Grab whatever the filters are showing.',
        ],
        'import' => [
            'label' => 'Import CSV',
            'route' => 'admin.shift-types.import',
            'description' => 'Upload new or updated shift types via CSV.',
        ],
    ];

    protected function rules(Model $record = null): array
    {
        $slugRule = Rule::unique('shift_types', 'slug');
    ...
    public function import(Request $request)
    {
        ...
        foreach ($rows as $row) {
            ...
            ShiftType::updateOrCreate(['slug' => $slug], $payload);
            $imported++;
        }

        return back()->with('status', "Imported {$imported} shift types.");
    }

    public function export(Request $request)
    {
        $columns = ['slug', 'label', 'start_time', 'end_time', 'sort_order', 'is_active'];
        $filename = 'shift-types-' . Carbon::now()->format('Y-m-d') . '.csv';
    ...
    {
        $columns = ['slug', 'label', 'start_time', 'end_time', 'sort_order', 'is_active'];
        $filename = 'shift-types-sample.csv';

        $callback = function () use ($columns) {
            $handle = fopen('php://output', 'w');
            fputcsv($handle, $columns);
            fputcsv($handle, ['day', 'Day', '08:00', '18:00', 1, 1]);
            fputcsv($handle, ['night', 'Night', '20:00', '06:00', 2, 1]);
            fclose($handle);
        };
