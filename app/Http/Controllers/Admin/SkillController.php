...
class SkillController extends Controller
{
    ...
    {
        $skills = $this->buildQuery($request)
            ->paginate(15)
            ->withQueryString();
    ...
    public function import(Request $request)
    {
        ...
        foreach ($rows as $row) {
            ...
            Skill::updateOrCreate(['slug' => $slug], $payload);
            $inserted++;
        }

        return back()->with('status', "Imported {$inserted} skills.");
    }

    public function export(Request $request)
    {
        $skills = $this->buildQuery($request)->get();
        $columns = ['slug', 'label', 'sort_order', 'is_active'];
        $filename = 'skills-' . Carbon::now()->format('Y-m-d') . '.csv';

        $callback = function () use ($skills, $columns) {
    ...
    public function sample(): StreamedResponse
    {
        ...
        $callback = function () use ($columns) {
            $handle = fopen('php://output', 'w');
            fputcsv($handle, $columns);
            fputcsv($handle, ['frontend', 'Frontend', 10, 1]);
            fclose($handle);
        };
    ...
    {
        $this->ensureSlug($request);
        $data = $this->validateSkill($request, $skill);
        $data['is_active'] = $request->boolean('is_active', true);
        $data['sort_order'] = $data['sort_order'] ?? 0;
    ...
    {
        $uniqueRule = Rule::unique('skills', 'slug');
        if ($skill) {
            $uniqueRule->ignore($skill);
        }
