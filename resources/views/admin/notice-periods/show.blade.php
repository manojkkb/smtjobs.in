...
@section('title', $noticePeriod->label)
...
<div class="space-y-6">
    <header class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <h1 class="text-2xl font-semibold text-slate-900">Notice Period Details</h1>
        </div>
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('admin.notice-periods.edit', 1) }}" class="inline-flex items-center gap-2 rounded-2xl border border-slate-300 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-100">
                Edit Notice Period
            </a>
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <p class="text-sm text-slate-500">Highlight the most important facts about this Notice Period. Extend the list with business-critical attributes.</p>
        <dl class="mt-6 grid gap-5 sm:grid-cols-2">
            <div>
                <dt class="text-xs uppercase tracking-[0.3em] text-slate-500">Name</dt>
                <dd class="mt-2 text-lg font-semibold text-slate-900">Sample Notice Period</dd>
            </div>
            <div>
