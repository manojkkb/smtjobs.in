...
<div class="space-y-6">
    <header class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <h1 class="text-2xl font-semibold text-slate-900">Edit Employment Type</h1>
            <p class="text-sm text-slate-500">Update the master employment type details below.</p>
        </div>
        <a href="{{ route('admin.employment-types.index') }}" class="inline-flex items-center gap-2 rounded-2xl border border-slate-300 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:bg-slate-100">
            Back to employment types
        </a>
    </header>

    <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <form action="{{ route('admin.employment-types.update', $employmentType) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input
                        ...
                        name="label"
                        type="text"
                        value="{{ old('label', $employmentType->label) }}"
                        required
                ...
                <div>
                    ...
                    <input
                        ...
                        name="slug"
                        type="text"
                        value="{{ old('slug', $employmentType->slug) }}"
                        class="mt-1 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-slate-400 focus:outline-none"
                    >
                    @error('slug')
                        <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input
                        ...
                        name="sort_order"
                        type="number"
                        value="{{ old('sort_order', $employmentType->sort_order) }}"
                        class="mt-1 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-slate-400 focus:outline-none"
                    >
                </div>
                <div class="flex items-center gap-4">
                    <label class="inline-flex items-center gap-2 text-sm font-medium text-slate-600">
                        ...
                        <input type="checkbox" name="is_active" value="1" class="h-4 w-4 rounded border-slate-300 text-slate-900" @checked(old('is_active', $employmentType->is_active))>
                        Active
                    </label>
                    <p class="text-xs text-slate-400">Toggle visibility in the builder.</p>
                </div>
            </div>
